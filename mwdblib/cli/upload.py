...
def upload_params(fn):
    @click.option(
        ...
        type=HashFile(),
        default=None,
        help="Parent file or object hash (optional)",
    )
    @click.option(
        "--private", is_flag=True, default=False, help="Don't share object with anyone"
    )
    @click.option(
        "--public",
        is_flag=True,
        default=False,
        help="Make object and all descendants public",
    )
    @click.option(
        ...
        if (
            sum(
                (
                    ...
                    bool(kwargs.get("share_with")),
                )
            )
            > 1
        ):
            raise click.exceptions.UsageError(
                "'private', 'public' and 'share-with' options are mutually exclusive"
            )
...
@click.option(
    "--name",
    default=None,
    help="Original file name (default: base name from provided path)",
)
@upload_params
@confirm_action
@pass_mwdb
...
@click.option(
    "--name",
    default=None,
    help="Original blob name (default: base name from provided path)",
)
@upload_params
@confirm_action
@pass_mwdb
