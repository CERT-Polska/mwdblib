...
class TabularFormatter(ObjectFormatter):
    ...
    def format_object_list(self, objects):
        return self.format_table(
            ...
            widths=[[66, 12, 10, 24, 2], [8, 12, 10, 12, 0]],
    ...
    def format_file_row(self, file):
        return [
            "{}\n{}".format(BoldFormatter().format(self, file.name), file.sha256),
            SizeFormatter().format(self, file.size),
            "{}\n{}".format(file.type, TagFormatter().format(self, file.tags)),
    ...
    def format_config_row(self, config):
        return [
            "{}\n{}".format(BoldFormatter().format(self, config.family), config.id),
            config.type,
        ...
        for config_key in sorted(config.config_dict):
            attributes += [
                [
                    config_key,
                    AttributeFormatter(),
                    json.dumps(config.config_dict[config_key]),
                ]
            ]
        attributes += [
    ...
    def format_blob_row(self, blob):
        return [
            "{}\n{}".format(BoldFormatter().format(self, blob.name), blob.id),
            blob.type,
    ...
    def print_confirmation(self, message, **params):
        click.echo(
            message.format(
                **{
                    k: (
                        TagFormatter().format(self, [v])
                        if k == "tag"
                        else BoldFormatter().format(self, v)
                    )
