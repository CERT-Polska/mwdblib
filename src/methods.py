...
class MWDBMethods(object):
    """
    Implementation of MWDB API methods
    """
    ...
    def _convert_bytes(data):
        if isinstance(data, dict):
            return dict(map(MWDBMethods._convert_bytes, data.items()))

        if isinstance(data, bytes):
            return data.decode('utf-8', 'replace')

        if isinstance(data, (tuple, list)):
            return list(map(MWDBMethods._convert_bytes, data))
